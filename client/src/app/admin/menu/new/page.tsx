...
export default function AdminMenuNewPage() {
  ...
  useEffect(() => {
    api
      .get('/categories')
      .then((res) => setCategories(res.data.data?.categories || res.data.data || []))
      .catch(console.error);
  }, []);
  ...
  const handleMultipleFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((file, index) => {
      ...
      newItems.push({ id, file, url, isPrimary });
    });
  ...
  const handleAddCustomUrl = () => {
    ...
    setImagesList((prev) => [...prev, { id, url: customImageUrl.trim(), isPrimary }]);
    setCustomImageUrl('');
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-brand-surface-light rounded-xl border border-white/10 p-6 sm:p-8">
        <h1 className="text-2xl font-bold text-brand-cream mb-6">Add Menu Item</h1>
